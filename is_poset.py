...
def to_string(a, n = N, with_gaps = False):
    if a == 0:
        return "(/)".center(n)    

    pows = [1 << i for i in range(n)]

    pows[0], pows[3] = pows[3], pows[0]
...
def check_without(le_idx, elem_idx):
    ...
    try:
        is_poset(extensions_without, N-1)
        elem = bin(extensions[le_idx][elem_idx])
        print(f"It is a correct partial realizer of B_{N-1} without one element - {elem}")
